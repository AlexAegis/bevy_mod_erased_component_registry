...
pub trait EntityCommandInsertErasedComponentByTypeIdExtension {
	...
	fn insert_component_by_type_id(&mut self, type_id: TypeId) -> &mut Self;
}

impl EntityCommandInsertErasedComponentByTypeIdExtension for EntityCommands<'_> {
	fn insert_component_by_type_id(&mut self, type_id: TypeId) -> &mut Self {
		self.queue(insert_component_by_type_id(type_id));
		self
	}
}

fn insert_component_by_type_id(type_id: TypeId) -> impl EntityCommand {
